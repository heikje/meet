...
class ABTCalendarEvent(models.Model):
    ...
    abt_google_solution = fields.Selection([
        ('none', 'None'),
    ...
    abt_google_meet_conference_code = fields.Char(
        'Google Meet Code',
        readonly=True,
        store=True,
        copy=False,
    ...
    abt_google_conference_signature = fields.Char(
    ...
    abt_google_conference_uri = fields.Char(
    ...
    abt_google_conference_entry_point_type = fields.Char(
        'Entry Point Type',
        readonly=True,
        store=True,
        copy=False,
    ...
    abt_conference_requestId = fields.Char(
    ...
    def _generate_conference_request_id(self):
        """
        If the event has no Google Conference ID generate an ID add save it on abt_conference_requestId field
        """
        self.ensure_one()

        # If no ID is defined for this event then create a new one
        if not self.abt_conference_requestId:
            # Generate random & unique Conference ID
            request_id = self._generate_random_unique_conference_request_id(self.id)

            # Save the Conference Request ID on abt_conference_requestId field
            self.write({
                'abt_conference_requestId': request_id
            })
        ...
        return self.abt_conference_requestId
    ...
    def is_google_conference(self):
        ...
        """
        self.ensure_one()
        return self.abt_google_solution not in ('none', False)
    ...
    def _update_event_conference_data(self, conference_data):
        ...
        if conference_data:
            ...
            self.write({
                'abt_google_meet_conference_code': conference_data.get('conferenceId'),
                'abt_google_conference_signature': conference_data.get('signature'),
                'abt_google_conference_uri': entry_point.get('uri'),
                'abt_google_conference_entry_point_type': entry_point.get('entryPointType'),
                'abt_google_solution': conference_data.get('conferenceSolution', {}).get('key', {}).get('type',
                                                                                                        'none'),
            })
